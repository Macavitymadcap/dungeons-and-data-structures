...
> short term. It is fair in the mathematical sense, which is quite different from being kind."
>
...
core resolution mechanic: roll it, add a bonus, compare to a target number.[^1] The system has
...
but a Hope and Fear economy at the table level: two parallel resource pools that belong partly to the player and partly to the GM.[^3]
...
from true randomness for most purposes.[^2] For a gamebook, this is entirely sufficient.
...
twenty die faces that produce a winning total.[^4] There is no mystery here, only arithmetic.
...
of the most studied phenomena in game design, and it has no clean solution.[^5]
...
it is wrong. The dice are not keeping score.[^6]
...
skill, luck, and narrative tension.[^7]
...
disadvantage exert the most influence.[^8]
...
[^1]: *Chainmail* (1971) by Gary Gygax and Jeff Perren was a medieval miniatures wargame published
...
[^2]: The algorithm underlying `Math.random()` is not specified by the JavaScript standard and
...
[^4]: The general formula is: if your modifier is `m` and the DC is `d`, then the probability
...
[^5]: This is the subject of an entire literature in game studies. Jesse Schell's *The Art of
...
[^6]: The gambler's fallacy is the belief that a random process is somehow "due" a result it
...
[^7]: The *Fighting Fantasy* series uses a 2d6 system for combat: both sides roll two dice and
...
[^8]: The exact probabilities for advantage and disadvantage can be worked out from first
