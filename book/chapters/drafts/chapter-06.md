...
from true randomness for most purposes.[^1] For a gamebook, this is entirely sufficient.
...
twenty die faces that produce a winning total.[^2] There is no mystery here, only arithmetic.
...
of the most studied phenomena in game design, and it has no clean solution.[^3]
...
it is wrong. The dice are not keeping score.[^4]
...
skill, luck, and narrative tension.[^5]
...
disadvantage exert the most influence.[^6]
...
- A `RandomSource` type and injectable random function that makes all dice testable.
- `rollDie` for rolling a single die of any denomination.
- `rollD20Check` for d20 ability checks, skill checks, and saving throws, with full support
  for normal, advantage, and disadvantage modes.
- `rollDamage` for damage notation like `1d6+2` or `2d8`.
- A `RollResult` interface and a `DamageRollResult` interface that record every detail of
  a roll: all dice thrown, the die that counted, the modifier, the total, the DC, and the
  outcome.
- Roll results stored in the game log with enough rendered detail for the player to verify
  each outcome.
- Tests that cover modifier effects, DC comparisons, advantage, disadvantage, and
  deterministic damage rolls.

These live in `src/gamebook/rules/dice.ts`, with tests in
`src/gamebook/rules/dice.test.ts`. The choice resolution in `src/gamebook/play.ts` calls
`rollD20Check` to resolve any choice that has a `check` attached. The combat loop in Chapter
7 will use both `rollD20Check` and `rollDamage`.
...
[^1]: The algorithm underlying `Math.random()` is not specified by the JavaScript standard and
...
[^2]: The general formula is: if your modifier is `m` and the DC is `d`, then the probability
...
[^3]: This is the subject of an entire literature in game studies. Jesse Schell's *The Art of
...
[^4]: The gambler's fallacy is the belief that a random process is somehow "due" a result it
hasn't produced recently. It arises from a genuine statistical truth — in the long run,
outcomes do approach their expected frequencies — combined with a fundamental misunderstanding
...
[^5]: The *Fighting Fantasy* series uses a 2d6 system for combat: both sides roll two dice and
...
[^6]: The exact probabilities for advantage and disadvantage can be worked out from first
